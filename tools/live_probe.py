#!/usr/bin/env python3
"""Instrument the LIVE site trigger: does 'enter' fire? Is the tween alive?"""
...
profile=os.path.join(TMP,'mec-ox2-live')
...
print(js("""(function(){
  ...
  window.__ev=[];
  ['enter','enterBack','refresh','refreshInit'].forEach(function(ev){
    st.addEventListener(ev,function(){window.__ev.push(ev+'@'+Math.round(scrollY));});
  });
  window.__g=g;
  return JSON.stringify({
    ...
    tweenCount: gsap.getTweensOf(g.querySelectorAll(':scope > [data-reveal-child]')).length,
    gridTweenCount: gsap.getTweensOf(g).length
  });
  }catch(e){return 'ERR:'+e.message;}
})()"""))
js("window.__g.scrollIntoView({block:'center',behavior:'instant'})")
try: js('new Promise(r=>setTimeout(r,2000))',True)
except Exception: pass
print(js("""(function(){
  try{
  const g=window.__g;
  const st=ScrollTrigger.getAll().find(function(t){return t.trigger===g});
  return JSON.stringify({events:window.__ev, progress:+(st.progress||0).toFixed(3),
    isActive:st.isActive, scrollY:Math.round(scrollY),
    cellOp:getComputedStyle(g.querySelector(':scope > [data-reveal-child]')).opacity,
    gridCls:g.className});
  }catch(e){return 'ERR:'+e.message;}
})()"""))
proc.kill()
