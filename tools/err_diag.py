#!/usr/bin/env python3
"""Capture JS errors + trigger state to find why clampedStart kills reveals."""
...
profile=os.path.join(TMP,'mec-ox2-errdiag')
...
def js(expr,**kw):
    ...
    if exc:
        return 'EXC: '+json.dumps(exc)[:300]
    return res.get('value')
cmd('Page.enable'); cmd('Runtime.enable')
cmd('Page.addScriptToEvaluateOnNewDocument',{'source':
  "window.__errs=[];window.addEventListener('error',function(e){window.__errs.push(e.message+' @ '+(e.filename||'').split('/').pop()+':'+e.lineno+':'+e.colno)});"})
time.sleep(3)
print('errors:', js("JSON.stringify(window.__errs)"))
print('stCount:', js("ScrollTrigger.getAll().length"))
print('maxScroll fn:', js("typeof ScrollTrigger.maxScroll"))
print('manual call:', js("(function(){var g=document.querySelector('.feat-grid');var top=g.getBoundingClientRect().top+(window.pageYOffset||0);var ideal=top-innerHeight*0.86;var max=ScrollTrigger.maxScroll(window);return JSON.stringify({top:Math.round(top),ideal:Math.round(ideal),max:Math.round(max),clamped:Math.round(Math.max(0,Math.min(ideal,max-2)))})})()"))
# inspect one trigger's stored start value type
print('trig starts:', js("JSON.stringify(ScrollTrigger.getAll().slice(0,3).map(function(t){return {start:t.start, typ:typeof t.start}}))"))
proc.kill()
