#!/usr/bin/env python3
"""Test B: function-based start returning a NUMBER (not string)."""
...
profile=os.path.join(TMP,'mec-ox2-fnstart2')
...
print(js("""(function(){
  ...
  gsap.fromTo(g,{opacity:0,y:28},{opacity:1,y:0,duration:.5,ease:'power2.out',
    scrollTrigger:{trigger:g,start:function(){return Math.max(0,Math.min(g.getBoundingClientRect().top+window.pageYOffset-innerHeight*0.86, ScrollTrigger.maxScroll(window)-2));},once:true}});
  ScrollTrigger.refresh();
  const st=ScrollTrigger.getAll().filter(function(t){return t.trigger===g});
  return JSON.stringify(st.map(function(t){return {start:t.start,end:t.end,isActive:t.isActive}}));
})()"""))
js("window.scrollTo({top:99999,behavior:'instant'})")
try: js('new Promise(r=>setTimeout(r,1500))',True)
except Exception: pass
print('opacity after full scroll:', js("getComputedStyle(document.querySelector('.feat-grid')).opacity"))
proc.kill()
