#!/usr/bin/env python3
"""Compare tween state on cta-band (WORKS - site-created, string start) vs
feat-cell (BROKEN - fn start). Same probe both."""
...
profile=os.path.join(TMP,'mec-ox2-cmp')
...
time.sleep(2)
js("window.scrollTo({top:999999,behavior:'instant'})")
try: js('new Promise(r=>setTimeout(r,1800))',True)
except Exception: pass
print(js("""JSON.stringify({
  band:{op:getComputedStyle(document.querySelector('.cta-band')).opacity,
        cls:document.querySelector('.cta-band').className,
        tweens:gsap.getTweensOf(document.querySelector('.cta-band')).length},
  cell:{op:getComputedStyle(document.querySelector('.feat-cell')).opacity,
        cls:document.querySelector('.feat-cell').className,
        tweens:gsap.getTweensOf(document.querySelector('.feat-cell')).length,
        inline:document.querySelector('.feat-cell').style.opacity}
})"""))
# force-play the cell tween manually — does the visual appear?
print(js("""(function(){
  try{
  const tw=gsap.getTweensOf(document.querySelector('.feat-cell'))[0];
  tw.play();
  return 'playing';
  }catch(e){return 'ERR:'+e.message;}
})()"""))
time.sleep(1.0)
print(js("""JSON.stringify({op:getComputedStyle(document.querySelector('.feat-cell')).opacity,
  prog:(function(){const t=gsap.getTweensOf(document.querySelector('.feat-cell'))[0];return t?+t.progress().toFixed(2):null})()})"""))
proc.kill()
