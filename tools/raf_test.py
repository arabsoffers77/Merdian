#!/usr/bin/env python3
"""THE decisive test: is the rAF loop alive in headless? Compare a plain
time-based tween (no scrolltrigger) vs scrolltrigger tween firing."""
...
profile=os.path.join(TMP,'mec-ox2-rAF')
...
time.sleep(2)
# A) plain time-based tween on feat-grid opacity
print(js("""(function(){
  const g=document.querySelector('.feat-grid');
  gsap.fromTo(g,{opacity:0.01},{opacity:0.99,duration:.5,ease:'none',delay:0});
  return 'created';
})()"""))
try: js('new Promise(r=>setTimeout(r,1200))',True)
except Exception: pass
print('A plain tween final opacity:', js("getComputedStyle(document.querySelector('.feat-grid')).opacity"))
proc.kill()
