#!/usr/bin/env python3
"""Repro the user's exact viewport: 1366x619 logical (2x DPR screenshot 2732x1238).
Check whether feat-grid children reveal at this short viewport."""
...
profile = os.path.join(TMP, 'mec-ox2-vprepro')
...
dl=time.time()+15
while time.time()<dl:
    if js("document.querySelectorAll('.hex').length===3"): break
    time.sleep(0.4)
res = js("""(() => {
  ...
  return JSON.stringify({
    vh: innerHeight,
    gridTop: Math.round(g.getBoundingClientRect().top + scrollY),
    gridStart: st ? Math.round(st.start) : null,
    gridEnd: st ? Math.round(st.end) : null,
    gridProgress: st ? Math.round((st.progress||0)*100) : null,
    gridOpacity: getComputedStyle(g).opacity,
