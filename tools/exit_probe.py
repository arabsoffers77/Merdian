#!/usr/bin/env python3
"""Check: does the site think REDUCED=true or GSAP missing? (exit-path probe)"""
...
profile=os.path.join(TMP,'mec-ox2-exitpath')
...
print(js("""JSON.stringify({
  ...
  gsapObj: !!window.gsap, gsapTo: !!(window.gsap&&typeof window.gsap.to==='function'),
  stReg: !!(window.ScrollTrigger),
  stCount: window.ScrollTrigger?ScrollTrigger.getAll().length:-1,
  inCount: document.querySelectorAll('.is-in').length,
  revealCount: document.querySelectorAll('[data-reveal],[data-reveal-child]').length
})"""))
proc.kill()
