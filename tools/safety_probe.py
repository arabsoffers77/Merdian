#!/usr/bin/env python3
"""Check the safety-net tween on feat-grid children: trigger, start, state."""
...
profile=os.path.join(TMP,'mec-ox2-safety')
...
print(js("""(function(){
  ...
  const tweens=gsap.getTweensOf(cell);
  window.__g=g;
  return JSON.stringify(tweens.map(function(t){
    const st=t.scrollTrigger;
    return {stStart:st?Math.round(st.start):null,
            stEnd:st?Math.round(st.end):null,
            startStr:st&&typeof st.vars.start==='string'?st.vars.start:String(typeof st.vars.start),
            once:st?st.vars.once:null, paused:t.paused()};
  }));
...
print(js("""(function(){
  ...
  const cell=g.querySelector(':scope > [data-reveal-child]');
  const tweens=gsap.getTweensOf(cell);
  return JSON.stringify({scrollY:Math.round(scrollY), cellOp:getComputedStyle(cell).opacity,
    states:tweens.map(function(t){const st=t.scrollTrigger;
      return {prog:+(t.progress()||0).toFixed(3), paused:t.paused(),
              stProg:st?+(st.progress||0).toFixed(3):null, stActive:st?st.isActive:null};})});
  }catch(e){return 'ERR:'+e.message;}
})()"""))
proc.kill()
