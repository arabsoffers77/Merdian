#!/usr/bin/env python3
"""Inspect the feat-cell tween: trigger start type/value, paused, and whether
its ScrollTrigger.refresh() re-reads the fn. THE deep probe."""
...
profile=os.path.join(TMP,'mec-ox2-deeptween')
...
time.sleep(2)
# force refresh and watch the tween's ST start before/after + scroll past
print(js("""(function(){
  try{
  const cell=document.querySelector('.feat-cell');
  const tw=gsap.getTweensOf(cell)[0];
  const st=tw.scrollTrigger;
  const before={start:st.start,end:st.end,startType:typeof st.vars.start,
    startStr:typeof st.vars.start==='string'?st.vars.start:'fn'};
  ScrollTrigger.refresh();
  const after={start:st.start,end:st.end};
  return JSON.stringify({before,after,maxScroll:ScrollTrigger.maxScroll(window)});
...
print(js("""(function(){
  ...
  const tw=gsap.getTweensOf(cell)[0];
  const st=tw.scrollTrigger;
  return JSON.stringify({y:Math.round(scrollY),stStart:Math.round(st.start),
    prog:+tw.progress().toFixed(3),paused:tw.paused(),op:getComputedStyle(cell).opacity});
  }catch(e){return 'ERR:'+e.message;}
})()"""))
proc.kill()
