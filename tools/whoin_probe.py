#!/usr/bin/env python3
"""Check the 14 is-in elements: which are they? Are feat-grid kids among them?
Also: are the site's stagger tweens created at all (kidTweens>0 earlier said yes
but paused)? Compare tween counts per element."""
...
profile=os.path.join(TMP,'mec-ox2-whoin')
...
print(js("""JSON.stringify({
  inList: [...document.querySelectorAll('.is-in')].map(function(e){
    return (e.className||'').toString().split(' ').slice(0,2).join('.');
  }),
  gridKidTweenCount: gsap.getTweensOf(document.querySelector('.feat-grid :scope > [data-reveal-child], .feat-cell')).length,
  anyFeatCellTween: gsap.getTweensOf(document.querySelector('.feat-cell')).length,
  heroTitleSplit: !!document.querySelector('.hero-title span')
})"""))
proc.kill()
