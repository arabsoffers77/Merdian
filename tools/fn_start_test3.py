#!/usr/bin/env python3
"""Test C: does the tween fire at all? Track onEnter via a global flag,
and check whether the trigger's start/end look sane vs actual scroll."""
...
profile=os.path.join(TMP,'mec-ox2-fnstart3')
...
print(js("""(function(){
  ...
  gsap.fromTo(g,{opacity:0,y:28},{opacity:1,y:0,duration:.5,ease:'power2.out',
    scrollTrigger:{trigger:g,start:function(){return Math.round(g.getBoundingClientRect().top+window.pageYOffset-innerHeight*0.86)+' top';},once:true,
      onEnter:function(){window.__entered=true;}}});
  ScrollTrigger.refresh();
  const st=ScrollTrigger.getAll().filter(function(t){return t.trigger===g}).pop();
  window.scrollTo({top:st.start+500,behavior:'instant'});
  return JSON.stringify({startStr:st.start, end:st.end, scroller:!!ScrollTrigger.getAll()[0].scroller});
})()"""))
try: js('new Promise(r=>setTimeout(r,1500))',True)
except Exception: pass
print('entered flag:', js("window.__entered"))
print('opacity now:', js("getComputedStyle(document.querySelector('.feat-grid')).opacity"))
proc.kill()
