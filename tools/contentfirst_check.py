#!/usr/bin/env python3
"""CONTENT-FIRST proof: at 1366x619 (user's viewport), with ZERO scrolling,
the Selected Work grid must be fully opaque in computed style. Then wheel
down and confirm it stays visible + screenshot the section in-frame."""
...
TAG = 'mec-ox2-cf%d' % int(time.time())
...
def main():
    ...
    # THE decisive assertion: WITHOUT any scrolling, cards are opaque
    a=json.loads(ws.js("""(() => {
    ...
    ok_a = a['gridOp']=='1' and all(o==1 for o in a['cellOps'])
    print('NO-SCROLL state:', a, '->', 'PASS' if ok_a else 'FAIL')

    # scroll down like a user, then frame the section & screenshot
    for i in range(12):
        ws.cmd('Input.dispatchMouseEvent',
               {'type':'mouseWheel','x':700,'y':300,'deltaX':0,'deltaY':420})
        try: ws.js('new Promise(r=>setTimeout(r,350))',True)
        except Exception: pass
    ...
    b=json.loads(ws.js("""(() => {
      ...
      return JSON.stringify({inView:r.top<innerHeight&&r.bottom>0,
        gridOp:getComputedStyle(g).opacity,
        cellOps:cells.map(c=>+getComputedStyle(c).opacity)});
    })()"""))
    ok_b = b['gridOp']=='1' and all(o==1 for o in b['cellOps']) and b['inView']
    print('AFTER-SCROLL state:', b, '->', 'PASS' if ok_b else 'FAIL')
    shot=ws.cmd('Page.captureScreenshot',{'format':'png'})
    open(os.path.join(OUT,'ox2-selected-work-contentfirst.png'),'wb').write(base64.b64decode(shot['result']['data']))
    print('saved ox2-selected-work-contentfirst.png')
    print('CONTENT-FIRST:', 'ALL PASS' if (ok_a and ok_b) else 'FAIL')
    proc.kill()
