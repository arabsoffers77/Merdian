#!/usr/bin/env python3
"""Minimal: create a trigger with clampedStart-style fn start, scroll past,
does onEnter fire? (pure, no site JS interference)"""
...
profile=os.path.join(TMP,'mec-ox2-minst')
...
time.sleep(2)
# pure ScrollTrigger on a plain div with a NUMBER-returning function start
print(js("""(function(){
  window.__fired=false;
  const probe=document.createElement('div');
  probe.style.cssText='position:absolute;top:3000px;height:10px;width:10px';
  document.body.appendChild(probe);
  ScrollTrigger.create({trigger:probe,
    start:function(){return Math.max(0,Math.min(3000-innerHeight*0.86, ScrollTrigger.maxScroll(window)-2));},
    once:true,onEnter:function(){window.__fired=true;}});
  ScrollTrigger.refresh();
  const st=ScrollTrigger.getAll().filter(function(t){return t.trigger===probe}).pop();
  const before={start:st.start,end:st.end};
  window.scrollTo({top:999999,behavior:'instant'});
  return JSON.stringify(before);
})()"""))
try: js('new Promise(r=>setTimeout(r,1200))',True)
except Exception: pass
print('fired:', js("window.__fired"), '| scrollY:', js("Math.round(scrollY)"))
proc.kill()
