#!/usr/bin/env python3
"""Check the ticker (marquee) tween: is IT progressing in headless? If the
ticker moves but reveals don't, GSAP core works and it's trigger-callback
scheduling. If ticker is frozen too, something froze gsap.ticker."""
...
profile=os.path.join(TMP,'mec-ox2-ticker')
...
time.sleep(2)
# find any infinite/running tweens (ticker uses CSS anim? check transform over time)
x1 = js("(function(){const t=document.querySelector('.ticker-track')||document.querySelector('.ticker > *');return t?Math.round(t.getBoundingClientRect().left):null})()")
time.sleep(1.5)
x2 = js("(function(){const t=document.querySelector('.ticker-track')||document.querySelector('.ticker > *');return t?Math.round(t.getBoundingClientRect().left):null})()")
print('ticker x:', x1, '->', x2, '(moving)' if x1!=x2 else '(STATIC)')
# global lag/timing check
print(js("JSON.stringify({lag:gsap.ticker?undefined:'n/a', now:Math.round(performance.now())})"))
proc.kill()
