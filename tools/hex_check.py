#!/usr/bin/env python3
"""Verify hex cluster geometry + slide-text buttons roll on hover."""
...
def connect(url):
    profile = os.path.join(TMP, 'mec-ox2-hex-%d' % (time.time() * 1000 % 1e9))
...
if __name__ == '__main__':
    try:
        urllib.request.urlopen(BASE + '/index.html', timeout=2); print('server: up')
    except Exception:
        subprocess.Popen(['python', '-m', 'http.server', '8123', '--bind', '127.0.0.1'],
                         cwd=r'D:\work\merdian\ox 2',
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(1.5); print('server: started')

    proc, ws = connect(BASE + '/index.html')
    deadline = time.time() + 15
    while time.time() < deadline:
        if ws.js("document.querySelectorAll('.hex').length === 3"):
            break
        time.sleep(0.4)

    # hexes clipped and images loaded?
    hexinfo = json.loads(ws.js("""(() => {
      const out = [];
      document.querySelectorAll('.hex').forEach(h => {
        const img = h.querySelector('img');
        const r = h.getBoundingClientRect();
        out.push({clipped: getComputedStyle(h).clipPath.indexOf('polygon') !== -1
                           || getComputedStyle(h).clipPath.indexOf('url') !== -1,
                  w: Math.round(r.width), h: Math.round(r.height),
                  imgOk: img && img.complete && img.naturalWidth > 50});
      });
      return JSON.stringify(out);
    })()"""))
    print('HEXES:', hexinfo)

    # scroll-text button roll on hover (simulate with class since :hover needs pointer)
    before = ws.js("getComputedStyle(document.querySelector('.hero-ctas .st-row')).transform")
    ws.js("document.querySelector('.hero-ctas .btn').classList.add('is-test-hover')")
    time.sleep(0.55)
    after = ws.js("getComputedStyle(document.querySelector('.hero-ctas .st-row')).transform")
    rolled = before != after and after != 'none'
    ws.js("document.querySelector('.hero-ctas .btn').classList.remove('is-test-hover')")
    alt_label = ws.js("(document.querySelector('.hero-ctas .st-b')||{}).textContent")
    print(f"SLIDE BTN: rolledOnHover={rolled}, altLabel={alt_label!r}",
          "PASS" if rolled else "FAIL")

    # screenshot of the about-preview section with hexes
    ws.js("document.querySelector('.hex-cluster').scrollIntoView({block:'center'})")
    try:
        ws.js('new Promise(r => setTimeout(r, 2000))', await_promise=True)
    except Exception:
        pass
    shot = ws.cmd('Page.captureScreenshot', {'format': 'png'})
    with open(os.path.join(OUT, 'ox2-hex-cluster.png'), 'wb') as fh:
        fh.write(base64.b64decode(shot['result']['data']))
    print('saved ox2-hex-cluster.png')

    all_hex_ok = all(x['clipped'] and x['imgOk'] and x['w'] > 60 for x in hexinfo)
    print('OVERALL:', 'PASS' if all_hex_ok and rolled else 'FAIL')
    proc.kill()
